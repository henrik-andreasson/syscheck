...
for (( j=0; j < ${#REMOTE_HOSTNAME[@]} ; j++ )){
	...
	if [ $? -ne 0 ] ; then
      		printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[2]} "${DESCR[2]}" ${LOCAL_FILE[$j]} "file: ${LOCAL_FILE[$j]} to:${REMOTE_HOSTNAME[$j]} dir:${REMOTE_DIR[$j]} remotreuser:${REMOTE_USER[$j]} sshkey: ${SSHKEY[$j]} result: ${SSHCOPYRES}"
	else
		printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $INFO ${ERRNO[1]} "${DESCR[1]}" ${LOCAL_FILE[$j]} ${SSHCOPYRES}
	fi
}
