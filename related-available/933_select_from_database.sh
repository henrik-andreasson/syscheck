...
for (( j=0; j < ${#SQL_SELECT[@]} ; j++ )){
	...
	if [ $? -eq 0 ] ; then
	        printlogmess -n ${SCRIPTNAME} -i ${SCRIPTID} -x ${SCRIPTINDEX} -l $INFO -e ${ERRNO[1]} -d "${DESCR[1]}" -1 "${SQL_DESC[$j]}"
	else
	        printlogmess -n ${SCRIPTNAME} -i ${SCRIPTID} -x ${SCRIPTINDEX} -l $ERROR -e ${ERRNO[2]} -d "${DESCR[2]}" -1 "${SQL_DESC[$j]}"
	fi
