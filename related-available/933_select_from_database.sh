...
for (( j=0; j < ${#SQL_SELECT[@]} ; j++ )){
	...
	if [ $? -eq 0 ] ; then
	        printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $INFO ${ERRNO[1]} "${DESCR[1]}" "${SQL_DESC[$j]}"
	else
	        printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[2]} "${DESCR[2]}" "${SQL_DESC[$j]}"
	fi
