...
if [ ! -d ${InTransitDir} ] ; then
	printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[9]} "$ARCHIVE_DESCR[9]"
	exit -1
fi
...
FileToArchive=
if [ "x$1" = "x" ] ; then
	printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[3]} "$ARCHIVE_DESCR[3]"
	exit -1
else
    FileToArchive=$1
fi
...
moveToIntransit() {
	...
	if [ "x${IntransitFileName}" = "x" ] ; then
		printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[8]} "$ARCHIVE_DESCR[8]" ${IntransitFileName}
		exit -1
	fi
# move the file into the intransit dir and give it a unique name
	...
	if [ $? != 0 ] ; then
	 	printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[2]} "$ARCHIVE_DESCR[2]" ${file} ${InTransitDir}/${IntransitFileName}
		exit -1
	else
		printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $INFO ${ERRNO[7]} "$ARCHIVE_DESCR[7]" ${file} ${InTransitDir}/${IntransitFileName}
...
# claim the filename that the file is not already there
	...
	if [ $? != 0 ] ; then
                printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[4]} "$ARCHIVE_DESCR[4]"
		exit -1
	fi
...
# transfer the file
	...
	if [ $? != 0 ] ; then
                printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[5]} "$ARCHIVE_DESCR[5]" "${InTransitDir}/${IntransitFileName} $ArchiveServer ${ArchiveDir}/${remoteFileName}"
		exit -1
	fi
	...
	if [ $? != 0 ] ; then
                printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[5]} "$ARCHIVE_DESCR[5]" "md5sum check failed"
		exit -1
	fi
...
# return the filename
        printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $INFO ${ERRNO[6]} "$ARCHIVE_DESCR[6]" "${InTransitDir}/${IntransitFileName} $ArchiveServer ${ArchiveDir}/${remoteFileName}"
	echo "${remoteFileName}"
}
...
# move the file locally also
	...
	if [ $? != 0 ] ; then
	 	printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR ${ERRNO[2]} "$ARCHIVE_DESCR[2]" ${InTransitDir}/${IntransitFileName} ${ArchiveDir}/${remoteFileName}
		exit -1
	else
		printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $INFO ${ERRNO[1]} "$ARCHIVE_DESCR[1]" ${InTransitDir}/${IntransitFileName} ${ArchiveDir}/${remoteFileName}
	fi
}
