...
checkocsp() {
  ...
  while getopts ":u:i:c:h:a:s:d" opt; do
    case $opt in
     u)
        OCSP_URL="$OPTARG"
       ;;
  ...
  if [ "x$PRINTVERBOSESCREEN" != "x" ] ; then
        ...
        CERTINFO_SUBJECT=$(echo "$CERTINFO" | grep "^subject=CN"    | sed 's#^subject=##')
        CERTINFO_ISSUER=$(echo "$CERTINFO" | grep "^issuer=CN"      | sed 's#^issuer=##')
...
#    printverbose "### END OCSP INFO ###"
    ...
    if [ "x${OCSP_STATUS}" == "x${EXPECTED_STATUS}" ] ; then
      ...
      MESSAGE="${MESSAGE}; ${OCSP_RESPONDER_RESPONSE_STATUS}"
    fi

    OCSP_RESPONDER_CERT_CHECK_VALIDITY=$(${SYSCHECK_HOME}/lib/cmp_dates.py "$OCSP_RESPONDER_CERT_NOT_BEFORE" "$OCSP_RESPONDER_CERT_NOT_AFTER" --warnminutes "${WARN_OCSP_RESPONDER_CERT_EXPIRE}" --errorminutes "${ERROR_OCSP_RESPONDER_CERT_EXPIRE}" --minutes )
    ...
    if [ "x$OCSP_RESPONDER_CERT_CHECK_VALIDITY" = "x" ] ; then
      ...
      STATUS=$(expr $STATUS + 1)
      MESSAGE="${MESSAGE}; ${OCSP_RESPONDER_CERT_CHECK_RETCODE}"
...
# ocsp responder cert is before any warn/error time
      ...
      STATUS=$(expr $STATUS + 1)
      MESSAGE="${MESSAGE}; problem calculating validity"
    fi
...
for (( i = 0 ;  i < ${#OCSP_TEST[@]} ; i++ )) ; do
    ...
    # if [ "x${OCSP_HOST_NAME[$i]}" = "x" ] ; then
    #   OCSP_HOST_NAME_OPT="-h "
    # fi
    ...
    for ocsp_cert_to_test in ${OCSP_TEST[$i]}/test_*_cert* ; do
      ...
      printverbose "## Current test: cert: $ocsp_cert_to_test to URL: ${OCSP_URL[$i]} issuer: ${issuer_cert} cacerts: ${cacerts}"
      checkocsp  -c "${ocsp_cert_to_test}" -u "${OCSP_URL[$i]}" -i "${issuer_cert}" -a "${cacerts}" -s "${expected_status}"
    done
done
...
#     printlogmess -n ${SCRIPTNAME} -i ${SCRIPTID} -x ${SCRIPTINDEX} -l $ERROR -e ${ERRNO[2]} -d "${DESCR[9]}" -1 "${GLOBALERRMESSAGE}"
# elif [ "x${WARNSTATUS}" != "x0" ] ; then
#     printlogmess -n ${SCRIPTNAME} -i ${SCRIPTID} -x ${SCRIPTINDEX} -l $WARN  -e ${ERRNO[2]} -d "${DESCR[9]}" -1 "${GLOBALERRMESSAGE}"
# else
