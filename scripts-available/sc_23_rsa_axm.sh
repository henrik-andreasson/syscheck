...
if [ $NUMBER_OF_NOT_RUNNING_PROCS -gt 0 ] ; then
        printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $ERROR $RSA_AXM_ERRNO[2] "$RSA_AXM_DESCR[2]" $NAMES_OF_NOT_RUNNING_PROCS
	exit 2
else
        printlogmess ${SCRIPTNAME} ${SCRIPTID} ${SCRIPTINDEX}   $INFO $RSA_AXM_ERRNO[1] "$RSA_AXM_DESCR[1]"
	exit 0
fi
